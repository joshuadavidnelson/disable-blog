...
- **Bug fix:** RSS/Atom feeds requested via query string (e.g. `/?feed=rss2`) were not disabled and returned live post content with a 200 response, even though `/feed/` was correctly redirected. The check keyed off the global `$post` object, which a static front page substitutes with the Home page, causing the guard to bail out. The check is now based on the request's actual query vars instead.
- **Bug fix:** A removed users sitemap (e.g. `/wp-sitemap-users-1.xml`) returned a 200 response serving the normal template, which included post titles, instead of a 404. Removing a sitemap provider entirely leaves core with no provider to 404 against, and this plugin's own sitemap-redirect guard intentionally skips sitemap urls, so nothing else caught it. These requests now correctly 404. 
    - **New filter:** `dwpb_disable_removed_sitemaps` (default `true`) toggles this behavior. Note post and taxonomy sitemap subtypes were unaffected by this bug; core already 404s those correctly on its own.
- Fix `term.php` never redirecting, caused by a misspelled check method (`reidrect_admin_term()`); renamed to `redirect_admin_term()`.
- Fix `tools.php` (Available Tools) never redirecting; the dispatch loop looked for `redirect_admin_tools()` but the method was named `redirect_admin_options_tools()`. **Filter renamed:** the redirect url for this screen is now filtered via `dwpb_redirect_admin_tools` (previously documented as `dwpb_redirect_admin_options_tools`, which never actually fired because the redirect it belonged to never ran). The old filter name is still honored via `apply_filters_deprecated()` so existing code keeps working, but new code should use `dwpb_redirect_admin_tools`.
...
- Remove the `system.listMethods`, `system.multicall`, and `system.getCapabilities` entries from the XML-RPC disabled-methods list. WordPress core re-registers these methods after the plugin's `xmlrpc_methods` filter runs, so they were never actually removable this way and listing them implied otherwise; a comment now documents why. This is not a behavior change, those methods remain callable, exactly as before.
...
- Fix `dwpb_redirect_category_archive` and `dwpb_redirect_post_tag_archive` never firing on a category/tag archive request. `modify_taxonomies_arguments()` strips the `category`/`post_tag` taxonomy's `query_var` (among other public-facing args) whenever no other post type uses it, which stopped `is_category()` from ever resolving true for a `/category/<slug>/` request; the request instead matched the `blog_page` branch, so a site hooking either filter to send those archives somewhere custom got nothing, even though the redirect itself still landed on the front page as usual. `redirect_public_pages()` now also checks the raw `category_name`/`cat`/`tag`/`tag_id` query vars, which are unaffected by the taxonomy's public state. No change to the default (unfiltered) redirect target for either archive.
- Remove `dashboard_recent_drafts` and `dashboard_incoming_links` from `remove_dashboard_widgets()`. Neither is a metabox core registers on any supported WordPress version (5.9+): "Recent Drafts" renders inline inside the Quick Press widget's own callback rather than as its own postbox, and "Incoming Links" was removed from core entirely in WordPress 3.8, so `remove_meta_box()` for either was always a no-op. **Filters removed:** `dwpb_disable_dashboard_recent_drafts` and `dwpb_disable_dashboard_incoming_links`, which never had any effect, go with them.
