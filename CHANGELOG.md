...
- **Security fix:** RSS/Atom feeds requested via query string (e.g. `/?feed=rss2`) were not disabled and returned live post content with a 200 response, even though `/feed/` was correctly redirected. The check keyed off the global `$post` object, which a static front page substitutes with the Home page, causing the guard to bail out. The check is now based on the request's actual query vars instead.
- **Security fix:** A removed users sitemap (e.g. `/wp-sitemap-users-1.xml`) returned a 200 response serving the normal template, which included post titles, instead of a 404. Removing a sitemap provider entirely leaves core with no provider to 404 against, and this plugin's own sitemap-redirect guard intentionally skips sitemap urls, so nothing else caught it. These requests now correctly 404. **New filter:** `dwpb_disable_removed_sitemaps` (default `true`) toggles this behavior. Note post and taxonomy sitemap subtypes were unaffected by this bug; core already 404s those correctly on its own.
